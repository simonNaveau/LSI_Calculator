	...
	//Course price
	var lesson_price= new Array(); //Lesson price list
	lesson_price["None"]=0;
	lesson_price["CORE"]=365;

	var afternoon_option= new Array(); //Afternoon option price list
	afternoon_option["None"]=0;
	afternoon_option["IT"]=570;
	afternoon_option["BC"]=860;
	afternoon_option["IN5"]=640;
	afternoon_option["IN10"]=915;
	afternoon_option["C"]=640;
...
/////////////////////////////////////////////////////Computing part //////////////////////////////////////////////////////////
	...
	function computeTotal(){
		totalToPrint = 80+((summerWeeksNumber+normalWeeksNumber)*(normalLessonsPrice+optionLessonsPrice)
			)+(summerWeeksNumber*summerAccommodationPrice)+(normalWeeksNumber*normalAccommodationPrice)+optionPrice+examOption+((summerWeeksNumber+normalWeeksNumber)*lunchOption);
	    document.getElementById('totalPrice').innerHTML = "Total: £"+totalToPrint; //Change the value printed in the view
	}
	
	function setMorningLesson(){
		var selectedLesson = theForm.elements["morningLesson"];
		normalLessonsPrice = lesson_price[selectedLesson.value];

		if(theForm.elements["morningLesson"].value !== "None" ){
			document.getElementById("datesView").hidden = false
			document.getElementById("afternoonView").hidden = false
			if(normalWeeksNumber != 0 || summerWeeksNumber != 0){
				var price = ((summerWeeksNumber+normalWeeksNumber)*(normalLessonsPrice));
				document.getElementById('lessonReview').innerHTML = "- Morning Course: £"+price;
				console.log("- Morning Course: £"+price+" ("+summerWeeksNumber+"+"+normalWeeksNumber+" x £"+normalLessonsPrice)	
			}
		} 
		else {
	...
	function setAfternoonLesson(){
		var selectedLesson = theForm.elements["afternoonLesson"];
		optionLessonsPrice = afternoon_option[selectedLesson.value];
		if(theForm.elements["afternoonLesson"].value !== "None" ){
			document.getElementById("datesView").hidden = false
			document.getElementById("afternoonView").hidden = false
			if(normalWeeksNumber != 0 || summerWeeksNumber != 0){
				var price = ((summerWeeksNumber+normalWeeksNumber)*(optionLessonsPrice));
				document.getElementById('afternoonLessonReview').innerHTML = "- Afternoon Course: £"+price;
				console.log("- Afternoon Course: £"+price+" ("+summerWeeksNumber+"+"+normalWeeksNumber+" x £"+optionLessonsPrice+")")	
			}
		}
		else {
			document.getElementById('afternoonLessonReview').innerHTML = "";
		} 
		computeTotal();
	}
	...
	function setWeeks(){
		...
		if(normalWeeksNumber > 0 || summerWeeksNumber > 0){
			...
			price = ((summerWeeksNumber+normalWeeksNumber)*(optionLessonsPrice));
			document.getElementById('afternoonLessonReview').innerHTML = "- Afternoon Course: £"+price;
			console.log("- Afternoon Course: £"+price+" ("+summerWeeksNumber+"+"+normalWeeksNumber+" x £"+optionLessonsPrice+")")
